...
export function AdminSidebar({ badges = {} }: { badges?: Record<string, number> }) {
  ...
  return (
    <>
      {/* Mobile toggle */}
      <button
        className="lg:hidden fixed top-3.5 left-4 z-50 p-2 bg-white rounded-lg shadow-md border border-gray-200"
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-white border-r border-gray-100 z-40 transition-transform lg:translate-x-0 ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="h-14 flex items-center gap-2 px-6 border-b border-gray-100">
          <div className="w-7 h-7 rounded-md bg-[#1B2A4A] flex items-center justify-center">

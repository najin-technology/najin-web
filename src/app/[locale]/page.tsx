...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return createPageMetadata({
    ...
    descriptions: {
      ko: "경남 양산 우레탄 성형, 합성수지 가공, CNC 정밀가공, 금형 제작 전문기업 나진테크. 현대자동차, SK, GM Shanghai 등 주요 대기업 납품.",
      en: "NAJIN TECHNOLOGY - Urethane molding, synthetic resin, CNC machining, mold fabrication in Yangsan, Korea. Supplying Hyundai, SK, GM Shanghai and other major clients.",
      zh: "纳进科技 — 庆南梁山聚氨酯成型、合成树脂加工、CNC精密加工、模具制作专业企业。向现代汽车、SK、GM上海等主要大企业供货。",
    },
  });
}
    ...
    <>
      ...
      <section className="py-10 md:py-14 bg-surface-warm-50 border-y border-surface-warm-200">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-x-6 gap-y-8 md:gap-x-10 md:gap-y-10 items-stretch" data-animate="fade-up">
            {CLIENTS.map((client) => (
              <Link
                key={client.slug}
                href={`/clients/${client.slug}`}
                className="flex items-center justify-center h-16 md:h-20 w-full px-2 transition-opacity opacity-70 hover:opacity-100"
